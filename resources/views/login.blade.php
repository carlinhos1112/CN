    ...
    <div class="min-h-screen bg-background flex flex-col">
        ...
        <main class="flex-1 flex items-center justify-center px-4 py-8">
            <div class="w-full max-w-xl">
                ...
                <footer class="mt-6 text-center text-sm text-muted-foreground">&copy; {{date('Y')}} <strong>Governo Federal</strong>. Todos os direitos reservados.</footer>
            </div>
        </main>
    </div>
@endsection
    ...
    $(document).on('submit', '#form-login', function (e) {
        ...
        .then(resposta => {
            if (resposta.success) {
                ...
                setTimeout(() => {
                    window.location.href = "{{url('/informacoes')}}";
                }, 2000);
